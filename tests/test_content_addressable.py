"""Tests for the `content_addressable` Python extension.

These tests exercise the PyO3 face of the `content-addressable` crate. The
contract they pin down:

- Content addressing is **deterministic**: the same value always names itself
  the same way, and different values name themselves differently.
- A `ContentId` round-trips through both its byte form and its string form.
- Canonicalization is a property of the *codec*, not caller field order: two
  dicts built in different insertion orders encode to identical bytes.
- Python and the Rust core share one canonicalizer + one hasher, so the
  `ContentId` of a value equals the id of its own canonical bytes.
- dag-cbor encode/decode is a true round-trip for representable values.
"""

import hashlib
import json
import pathlib

import pytest

import content_addressable as ca
from content_addressable import (
    ContentId,
    RawContentId,
    content_id,
    from_canonical_dagcbor,
    to_canonical_dagcbor,
)


def _blake3_digest(data: bytes):
    """Return a 32-byte BLAKE3 digest using whatever is available, or None.

    Prefers stdlib `hashlib.blake3` (CPython 3.13+); falls back to the `blake3`
    PyPI package; returns None if neither is present (then the cross-language
    wire-shape test is skipped rather than forcing a test-only dependency).
    """
    try:
        return hashlib.blake3(data).digest()  # type: ignore[attr-defined]
    except (AttributeError, ValueError):
        pass
    try:
        import blake3 as _blake3

        return _blake3.blake3(data).digest()
    except ImportError:
        return None


# --------------------------------------------------------------------------- #
# Determinism of content addressing
# --------------------------------------------------------------------------- #


def test_content_id_is_deterministic():
    """Same value -> same ContentId string, every time."""
    value = {"name": "alpha", "attrs": {"zeta": 26, "alpha": 1}}
    id_a = content_id(value)
    id_b = content_id(value)
    assert str(id_a) == str(id_b)
    assert id_a == id_b
    assert hash(id_a) == hash(id_b)


def test_content_id_differs_for_different_values():
    """Distinct values -> distinct ContentIds."""
    a = content_id({"name": "hello"})
    b = content_id({"name": "world"})
    assert a != b
    assert str(a) != str(b)


def test_content_id_usable_as_dict_key():
    """__hash__/__eq__ agree, so a ContentId works as a set/dict key."""
    a1 = content_id({"k": 1})
    a2 = content_id({"k": 1})
    b = content_id({"k": 2})
    s = {a1, a2, b}
    assert len(s) == 2  # a1 and a2 collapse to one entry


# --------------------------------------------------------------------------- #
# ContentId round-trips
# --------------------------------------------------------------------------- #


def test_content_id_bytes_roundtrip():
    """to_bytes / from_bytes is a lossless round-trip."""
    original = content_id({"x": [1, 2, 3]})
    raw = original.to_bytes()
    assert isinstance(raw, bytes)
    restored = ContentId.from_bytes(raw)
    assert restored == original
    assert str(restored) == str(original)


def test_content_id_string_roundtrip():
    """str / parse is a lossless round-trip."""
    original = content_id({"x": [1, 2, 3]})
    text = str(original)
    restored = ContentId.parse(text)
    assert restored == original
    assert restored.to_bytes() == original.to_bytes()


def test_content_id_repr():
    cid = content_id({"a": 1})
    r = repr(cid)
    assert r.startswith("ContentId('")
    assert str(cid) in r


def test_content_id_v1_base32_string_shape():
    """CIDv1 default multibase string is base32-lower, prefixed 'b'."""
    cid = content_id({"a": 1})
    s = str(cid)
    assert s.startswith("b")  # base32-lower multibase prefix


# --------------------------------------------------------------------------- #
# Frozen presentation accessors (issue #6): digest_bytes / digest_hex
# --------------------------------------------------------------------------- #


def test_digest_bytes_is_raw_32_byte_blake3_hash():
    """digest_bytes() is the bare 32-byte BLAKE3 digest — no CID envelope."""
    cid = content_id({"a": 1})
    d = cid.digest_bytes()
    assert isinstance(d, bytes)
    assert len(d) == 32
    # It is the tail of to_bytes() after the 4-byte CID envelope prefix
    # (0x01 v1, 0x71 dag-cbor, 0x1e BLAKE3, 0x20 len-32).
    assert cid.to_bytes() == bytes([0x01, 0x71, 0x1E, 0x20]) + d
    assert cid.to_bytes()[-32:] == d


def test_digest_hex_is_64_char_lower_hex_no_prefix():
    """digest_hex() is the 'bare-digest-hex' form: 64 lower-hex chars, no prefix."""
    cid = content_id({"a": 1})
    h = cid.digest_hex()
    assert isinstance(h, str)
    assert len(h) == 64
    assert not h.startswith("0x")
    assert h == h.lower()
    assert all(c in "0123456789abcdef" for c in h)
    # It is exactly hex of digest_bytes() — NOT hex of the full CID.
    assert h == cid.digest_bytes().hex()
    # And it is strictly shorter than full-CID-bytes-hex, which it is the tail of.
    cid_hex = cid.to_bytes().hex()
    assert cid_hex.endswith(h)
    assert len(cid_hex) > len(h)
    assert cid_hex == "01711e20" + h


def test_digest_hex_empty_map_is_frozen_value():
    """The empty-map digest_hex is the documented frozen value (issue #6)."""
    cid = content_id({})
    assert (
        cid.digest_hex()
        == "1f94cbf313b3ce23257a7251ea0fc95a24556ea611e4f8f475e549971baedb02"
    )


def test_digest_forms_agree_with_no_rehash_bridge():
    """digest_bytes() round-trips through the no-rehash constructor.

    Wrapping a ContentId's own digest_bytes() via from_blake3_content_digest
    reproduces the same id — proving digest_bytes() is exactly the digest the
    CID carries.
    """
    cid = content_id({"x": [1, 2, 3]})
    rebuilt = ContentId.from_blake3_content_digest(cid.digest_bytes())
    assert rebuilt == cid
    assert rebuilt.digest_hex() == cid.digest_hex()


# --------------------------------------------------------------------------- #
# Canonical dag-cbor: order independence (canonical sorting reaches Python)
# --------------------------------------------------------------------------- #


def test_canonical_dagcbor_is_insertion_order_independent():
    """Same dict built two ways -> identical canonical bytes."""
    one = {}
    one["alpha"] = 1
    one["zeta"] = 26

    two = {}
    two["zeta"] = 26
    two["alpha"] = 1

    assert to_canonical_dagcbor(one) == to_canonical_dagcbor(two)
    # ...and therefore the same ContentId.
    assert content_id(one) == content_id(two)


def test_canonical_dagcbor_nested_order_independent():
    a = {"outer": {"b": 2, "a": 1}, "k": [1, 2, 3]}
    b = {"k": [1, 2, 3], "outer": {"a": 1, "b": 2}}
    assert to_canonical_dagcbor(a) == to_canonical_dagcbor(b)


# --------------------------------------------------------------------------- #
# Cross-language consistency: one Rust core under both paths
# --------------------------------------------------------------------------- #


def test_content_id_equals_hash_of_its_own_canonical_bytes():
    """content_id(x) == ContentId.from_canonical_bytes(to_canonical_dagcbor(x)).

    This proves the helper, the codec, and the hasher all share one Rust core:
    canonicalizing then hashing by hand reproduces the helper's id exactly.
    """
    value = {"name": "alpha", "attrs": {"zeta": 26, "alpha": 1}, "n": 7}
    canon = to_canonical_dagcbor(value)
    by_hand = ContentId.from_canonical_bytes(canon)
    via_helper = content_id(value)
    assert by_hand == via_helper
    assert str(by_hand) == str(via_helper)


def test_content_id_matches_independent_blake3_cidv1():
    """The CID is BLAKE3(canonical bytes) wrapped as a CIDv1 dag-cbor link.

    Reconstruct the multihash + CIDv1 binary form from a stdlib BLAKE3 digest
    and assert it equals ContentId.to_bytes(). This pins the exact wire shape:
    codec 0x71 (dag-cbor), multihash code 0x1e (BLAKE3), 32-byte digest.
    """
    value = {"hello": "world", "n": [1, 2, 3]}
    canon = to_canonical_dagcbor(value)

    digest = _blake3_digest(canon)
    if digest is None:
        pytest.skip("no BLAKE3 implementation available (need py3.13+ or `blake3`)")
    assert len(digest) == 32

    # multihash = varint(code=0x1e) || varint(len=32) || digest
    multihash = bytes([0x1E, 0x20]) + digest
    # CIDv1 = varint(version=1) || varint(codec=0x71) || multihash
    cid_bytes = bytes([0x01, 0x71]) + multihash

    cid = content_id(value)
    assert cid.to_bytes() == cid_bytes
    # And the round-trip from those bytes lands on the same id.
    assert ContentId.from_bytes(cid_bytes) == cid


# --------------------------------------------------------------------------- #
# No-rehash digest bridge (issue #10)
# --------------------------------------------------------------------------- #


def test_from_blake3_content_digest_wraps_without_rehashing():
    """ContentId.from_blake3_content_digest(d) wraps d as-is — no second hash.

    For a non-trivial 32-byte digest, wrapping it must differ from
    from_canonical_bytes(d) (which hashes d again). Proves the no-rehash door
    does NOT re-hash. Also checks it matches content_id(...) of the same bytes,
    which canonicalizes-then-hashes — a third, different path.
    """
    d = bytes(range(1, 33))  # [1, 2, ..., 32], a non-trivial digest
    assert len(d) == 32

    wrapped = ContentId.from_blake3_content_digest(d)

    # The wrapped CID carries the digest verbatim: the trailing 32 bytes of the
    # CIDv1 binary form are exactly d.
    assert wrapped.to_bytes()[-32:] == d
    # CID shape: 0x01 (v1) 0x71 (dag-cbor) 0x1e (BLAKE3) 0x20 (len 32) || d
    assert wrapped.to_bytes() == bytes([0x01, 0x71, 0x1E, 0x20]) + d

    # The no-rehash invariant: hashing d again (from_canonical_bytes) MUST
    # produce a different id.
    rehashed = ContentId.from_canonical_bytes(d)
    assert wrapped != rehashed

    # And it differs from content_id(d), which canonicalizes d to dag-cbor then
    # hashes — yet another distinct path that must not collide with the wrap.
    assert wrapped != content_id(d)

    # Round-trips through its own byte form.
    assert ContentId.from_bytes(wrapped.to_bytes()) == wrapped


def test_from_blake3_content_digest_converges_with_from_canonical_bytes():
    """from_blake3_content_digest(blake3(x)) == from_canonical_bytes(x).

    When the caller hashes the real content correctly, the no-rehash door lands
    on exactly the id the hashing door produces. Skipped if no host BLAKE3.
    """
    content = b"\x00\x01\x02 some canonical dag-cbor bytes"
    digest = _blake3_digest(content)
    if digest is None:
        pytest.skip("no BLAKE3 implementation available (need py3.13+ or `blake3`)")
    assert len(digest) == 32

    via_digest = ContentId.from_blake3_content_digest(digest)
    via_hash = ContentId.from_canonical_bytes(content)
    assert via_digest == via_hash
    assert str(via_digest) == str(via_hash)


def test_from_blake3_content_digest_rejects_wrong_length():
    """A Python bytes of the wrong length raises ValueError (no [u8;32] guard)."""
    for bad in (b"", b"too short", bytes(31), bytes(33), bytes(64)):
        with pytest.raises(ValueError):
            ContentId.from_blake3_content_digest(bad)


# --------------------------------------------------------------------------- #
# dag-cbor encode/decode round-trip
# --------------------------------------------------------------------------- #


def test_dagcbor_roundtrip_values():
    for value in [
        None,
        True,
        False,
        0,
        -1,
        42,
        "hello",
        b"\x00\x01\x02bytes",
        [1, "two", [3, 4], None],
        {"a": 1, "b": [True, False], "c": {"nested": "map"}},
        {},
        [],
    ]:
        encoded = to_canonical_dagcbor(value)
        assert isinstance(encoded, bytes)
        decoded = from_canonical_dagcbor(encoded)
        assert decoded == value, f"round-trip mismatch for {value!r}"


def test_dagcbor_float_roundtrip():
    value = {"pi": 3.14159, "e": 2.71828}
    decoded = from_canonical_dagcbor(to_canonical_dagcbor(value))
    assert decoded == value


def test_bytes_decode_as_python_bytes():
    """A bytes value decodes back as `bytes`, not a list of ints."""
    decoded = from_canonical_dagcbor(to_canonical_dagcbor(b"raw"))
    assert decoded == b"raw"
    assert isinstance(decoded, bytes)


def test_tuple_encodes_like_list():
    """Tuples are dag-cbor lists; they decode back as lists."""
    assert to_canonical_dagcbor((1, 2, 3)) == to_canonical_dagcbor([1, 2, 3])
    assert from_canonical_dagcbor(to_canonical_dagcbor((1, 2, 3))) == [1, 2, 3]


# --------------------------------------------------------------------------- #
# Error handling
# --------------------------------------------------------------------------- #


def test_parse_invalid_cid_string_raises_value_error():
    with pytest.raises(ValueError):
        ContentId.parse("not-a-cid")


def test_from_bytes_invalid_raises_value_error():
    with pytest.raises(ValueError):
        ContentId.from_bytes(b"\xff\xff\xff not a cid")


def test_unsupported_type_raises_type_error():
    class Custom:
        pass

    with pytest.raises(TypeError):
        to_canonical_dagcbor(Custom())  # a custom object is not representable


# --------------------------------------------------------------------------- #
# Module surface
# --------------------------------------------------------------------------- #


def test_module_all_and_doc():
    assert set(ca.__all__) == {
        "ContentId",
        "RawContentId",
        "to_canonical_dagcbor",
        "from_canonical_dagcbor",
        "content_id",
    }
    assert ca.__doc__


# --------------------------------------------------------------------------- #
# Shared golden-vector byte-parity gate (Rust <-> Python)
# --------------------------------------------------------------------------- #
#
# tests/vectors.json is the ONE source of truth, generated by the Rust core
# (the authority that the Python face delegates to) and consumed verbatim here.
# The Rust integration test (tests/conformance.rs) asserts the same three fields
# against the same file, so any future byte drift fails LOUDLY in BOTH languages.
#
# These tests must NOT skip: the expected bytes come from the file, so no host
# BLAKE3 dependency is needed to assert agreement (unlike
# test_content_id_matches_independent_blake3_cidv1, which skips when BLAKE3 is
# absent). The vectors pin only the currently-stable, JSON-expressible dag-cbor
# subset (null/bool/int/str/list/map + bytes via the `{"$bytes": "<hex>"}`
# escape). Floats and the not-yet-frozen ContentId serde repr are deliberately
# excluded; they join the vectors once those must-fix-gate items freeze.

_VECTORS_PATH = pathlib.Path(__file__).parent / "vectors.json"

#: The reserved key that escapes a CBOR byte string inside JSON. Must match
#: ``BYTES_TAG`` in tests/common/mod.rs on the Rust side.
_BYTES_TAG = "$bytes"


def _expand_vector_value(value):
    """Expand a vector's JSON ``value`` into the native Python value it denotes.

    Mirrors ``json_value_to_ipld`` in tests/common/mod.rs: a single-key object
    ``{"$bytes": "<hex>"}`` becomes ``bytes``; everything else maps structurally.
    ``json.load`` already yields dict/list/int/str/bool/None for the rest of the
    subset, so only the bytes escape needs handling.
    """
    if isinstance(value, dict):
        if len(value) == 1 and _BYTES_TAG in value:
            return bytes.fromhex(value[_BYTES_TAG])
        return {k: _expand_vector_value(v) for k, v in value.items()}
    if isinstance(value, list):
        return [_expand_vector_value(v) for v in value]
    return value


def _load_vectors():
    with _VECTORS_PATH.open(encoding="utf-8") as fh:
        return json.load(fh)


_VECTORS = _load_vectors()


def test_vectors_file_present_and_covers_required_cases():
    """The shared file exists and carries the issue's required coverage."""
    assert len(_VECTORS) >= 8, f"need >= 8 vectors, found {len(_VECTORS)}"
    names = {v["name"] for v in _VECTORS}
    for required in (
        "empty_map",
        "empty_list",
        "nested_map",
        "map_order_independent",
        "ints_strings_bytes_mix",
        "sample_a",
    ):
        assert required in names, f"vectors.json must contain {required!r}"


@pytest.mark.parametrize("vector", _VECTORS, ids=[v["name"] for v in _VECTORS])
def test_golden_vector_matches_python_face(vector):
    """Every golden vector reproduces byte-for-byte through the Python face.

    Asserts the three pinned fields — canonical dag-cbor hex, ContentId string,
    and ContentId bytes hex — against the same vectors.json the Rust gate uses.
    Does NOT skip: agreement is asserted from the committed file, not a host hash.
    """
    value = _expand_vector_value(vector["value"])

    canon = to_canonical_dagcbor(value)
    assert canon.hex() == vector["canonical_dagcbor_hex"], (
        f"[{vector['name']}] canonical dag-cbor hex drifted"
    )

    cid = content_id(value)
    assert str(cid) == vector["content_id_str"], (
        f"[{vector['name']}] ContentId string drifted"
    )
    assert cid.to_bytes().hex() == vector["content_id_bytes_hex"], (
        f"[{vector['name']}] ContentId bytes drifted"
    )

    # The named presentation accessor digest_hex (issue #6) — the
    # bare-digest-hex form — must match the pinned value byte-for-byte, just as
    # the Rust gate asserts. This is what enforces cross-language parity for the
    # new accessor.
    assert cid.digest_hex() == vector["digest_hex"], (
        f"[{vector['name']}] digest_hex drifted"
    )
    # And digest_bytes() is the raw form of the same digest.
    assert cid.digest_bytes().hex() == vector["digest_hex"], (
        f"[{vector['name']}] digest_bytes() disagrees with pinned digest_hex"
    )
    # Structural cross-check: digest_hex is the CID-bytes tail after the prefix.
    assert vector["content_id_bytes_hex"] == "01711e20" + vector["digest_hex"], (
        f"[{vector['name']}] digest_hex must be the CID-bytes tail"
    )

    # The pinned bytes must also parse back to the same id (cross-check).
    assert ContentId.from_bytes(bytes.fromhex(vector["content_id_bytes_hex"])) == cid


# --------------------------------------------------------------------------- #
# RawContentId — the raw profile (issue #84)
# --------------------------------------------------------------------------- #
#
# tests/raw_vectors.json is generated by the Rust core (tests/gen_raw_vectors.rs)
# and consumed verbatim here and by tests/raw_conformance.rs, so the raw profile
# is byte-identical across both faces. It also pins law 6: the dag-cbor
# ContentId over the *same digest* is a different identity.

_RAW_VECTORS_PATH = pathlib.Path(__file__).parent / "raw_vectors.json"


def _load_raw_vectors():
    with _RAW_VECTORS_PATH.open(encoding="utf-8") as fh:
        return json.load(fh)


_RAW_VECTORS = _load_raw_vectors()


def test_raw_vectors_present():
    assert len(_RAW_VECTORS) >= 6
    names = {v["name"] for v in _RAW_VECTORS}
    for required in ("empty", "hello", "all_byte_values", "dagcbor_empty_map_as_raw_bytes"):
        assert required in names, f"raw_vectors.json must contain {required!r}"


@pytest.mark.parametrize("vector", _RAW_VECTORS, ids=[v["name"] for v in _RAW_VECTORS])
def test_raw_golden_vector_matches_python_face(vector):
    content = bytes.fromhex(vector["content_hex"])
    rid = RawContentId.from_content(content)
    assert str(rid) == vector["raw_content_id_str"]
    assert rid.to_bytes() == bytes.fromhex(vector["raw_content_id_bytes_hex"])
    assert rid.digest_hex() == vector["digest_hex"]
    assert rid.digest_bytes() == bytes.fromhex(vector["digest_hex"])
    assert rid.verify(content)
    assert not rid.verify(content + b"x")
    # Envelope prefix: CIDv1 (01) raw (55) blake3 (1e) 32-byte digest (20).
    assert vector["raw_content_id_bytes_hex"].startswith("01551e20")
    # Every ingress path agrees.
    assert RawContentId.parse(vector["raw_content_id_str"]) == rid
    assert RawContentId.from_bytes(rid.to_bytes()) == rid
    assert RawContentId.from_blake3_digest(rid.digest_bytes()) == rid


@pytest.mark.parametrize("vector", _RAW_VECTORS, ids=[v["name"] for v in _RAW_VECTORS])
def test_raw_and_dagcbor_profiles_never_compare_equal(vector):
    """Law 6: same digest, different profile => different identity."""
    rid = RawContentId.parse(vector["raw_content_id_str"])
    cid = ContentId.parse(vector["content_id_of_same_digest_str"])
    assert rid.digest_hex() == cid.digest_hex()
    assert str(rid) != str(cid)
    assert rid.to_bytes() != cid.to_bytes()
    # Cross-type comparison is False (not an error) and hashes need not collide.
    assert (rid == cid) is False
    assert (cid == rid) is False
    # Each parser refuses the other profile.
    with pytest.raises(ValueError):
        RawContentId.parse(vector["content_id_of_same_digest_str"])
    with pytest.raises(ValueError):
        ContentId.parse(vector["raw_content_id_str"])
    with pytest.raises(ValueError):
        RawContentId.from_bytes(cid.to_bytes())
    with pytest.raises(ValueError):
        ContentId.from_bytes(rid.to_bytes())


def test_raw_content_id_matches_independent_blake3_cidv1():
    """Independent construction: 0x01 0x55 0x1e 0x20 || blake3(content)."""
    blake3 = pytest.importorskip("blake3")
    content = b"independent raw check"
    digest = blake3.blake3(content).digest()
    assert RawContentId.from_content(content).to_bytes() == bytes([0x01, 0x55, 0x1E, 0x20]) + digest


def test_raw_content_id_rejects_wrong_digest_length_and_garbage():
    with pytest.raises(ValueError):
        RawContentId.from_blake3_digest(b"\x00" * 31)
    with pytest.raises(ValueError):
        RawContentId.parse("not a cid")
    with pytest.raises(ValueError):
        RawContentId.from_bytes(b"nope")


def test_raw_content_id_repr_hash_and_dict_key():
    a = RawContentId.from_content(b"k")
    b = RawContentId.from_content(b"k")
    assert repr(a).startswith("RawContentId('b")
    assert a == b and hash(a) == hash(b)
    assert {a: 1}[b] == 1


def test_from_dag_cbor_digest_is_the_explicit_successor():
    """ContentId.from_dag_cbor_digest == the (deprecated) from_blake3_content_digest."""
    cid = content_id({"a": 1})
    assert ContentId.from_dag_cbor_digest(cid.digest_bytes()) == cid
    assert ContentId.from_dag_cbor_digest(cid.digest_bytes()) == ContentId.from_blake3_content_digest(
        cid.digest_bytes()
    )
    with pytest.raises(ValueError):
        ContentId.from_dag_cbor_digest(b"\x00" * 8)


# --------------------------------------------------------------------------- #
# Portable legacy-dialect vectors (issue #84)
# --------------------------------------------------------------------------- #
#
# tests/legacy_vectors.json is generated by the Rust core
# (tests/gen_legacy_vectors.rs) and consumed by BOTH gates. The Rust gate
# (tests/legacy_conformance.rs) additionally drives the `unstable-legacy`
# adapters; Python checks the same identities with the API it has, which is what
# makes these conversions *portable* conformance data rather than Rust-only unit
# tests. Python also re-derives the SHA-256 digests with hashlib, so the
# `sha2-256:` rows are genuine nessie interop values, not synthetic shapes.

_LEGACY_VECTORS_PATH = pathlib.Path(__file__).parent / "legacy_vectors.json"


def _load_legacy_vectors():
    with _LEGACY_VECTORS_PATH.open(encoding="utf-8") as fh:
        return json.load(fh)


_LEGACY_VECTORS = _load_legacy_vectors()


def test_legacy_vectors_present():
    assert len(_LEGACY_VECTORS) >= 4
    names = {v["name"] for v in _LEGACY_VECTORS}
    assert {"empty", "hello", "quick_brown_fox"} <= names


@pytest.mark.parametrize(
    "vector", _LEGACY_VECTORS, ids=[v["name"] for v in _LEGACY_VECTORS]
)
def test_legacy_blake3_dialects_name_the_same_identity(vector):
    """kyln envelope-hex, nessie blake3:<hex>, and bare hex are one identity."""
    content = bytes.fromhex(vector["content_hex"])
    rid = RawContentId.from_content(content)
    assert str(rid) == vector["expect_raw_content_id_str"]
    # kyln-core's ContentId.to_hex() IS the CID envelope, hex-encoded.
    assert vector["kyln_envelope_hex"] == rid.to_bytes().hex()
    assert RawContentId.from_bytes(bytes.fromhex(vector["kyln_envelope_hex"])) == rid
    # nessie blake3 text and the bare hex carry the bare digest.
    assert vector["nessie_blake3_text"] == f"blake3:{rid.digest_hex()}"
    assert vector["bare_blake3_hex"] == rid.digest_hex()
    assert RawContentId.from_blake3_digest(bytes.fromhex(vector["bare_blake3_hex"])) == rid


@pytest.mark.parametrize(
    "vector", _LEGACY_VECTORS, ids=[v["name"] for v in _LEGACY_VECTORS]
)
def test_nessie_sha256_is_genuine_and_stays_foreign(vector):
    """`sha2-256:<hex>` is a real SHA-256 and converts to a non-mintable id."""
    content = bytes.fromhex(vector["content_hex"])
    algo, sha_hex = vector["nessie_sha256_text"].split(":")
    assert algo == "sha2-256"
    # The pinned digest really is SHA-256 of the content — a wrong literal in
    # the Rust generator fails HERE, so the vector cannot drift into fiction.
    assert sha_hex == hashlib.sha256(content).hexdigest()
    # The foreign CID is CIDv1(raw 0x55, sha2-256 0x12, 32 bytes) over it.
    assert vector["expect_foreign_cid_bytes_hex"] == "01551220" + sha_hex
    # Fail-closed: neither mintable profile will accept it, in either spelling.
    with pytest.raises(ValueError):
        RawContentId.parse(vector["expect_foreign_cid_str"])
    with pytest.raises(ValueError):
        ContentId.parse(vector["expect_foreign_cid_str"])
    with pytest.raises(ValueError):
        RawContentId.from_bytes(bytes.fromhex(vector["expect_foreign_cid_bytes_hex"]))
    with pytest.raises(ValueError):
        ContentId.from_bytes(bytes.fromhex(vector["expect_foreign_cid_bytes_hex"]))
    # And it is a different identity from the BLAKE3 one for the same content.
    assert vector["expect_foreign_cid_str"] != vector["expect_raw_content_id_str"]


def test_legacy_dialects_are_not_accepted_by_the_canonical_parsers():
    """ADVERSARIAL: the canonical parsers never learned the legacy dialects.

    Legacy spellings are read only through the explicit, default-off Rust
    adapters at a system's edge; teaching `parse` these forms would make a
    migration convenience into a permanent wire format (issue #84, D3).
    """
    v = _LEGACY_VECTORS[1]  # "hello"
    for dialect in (
        v["kyln_envelope_hex"],  # full-CID-bytes hex, no multibase prefix
        v["nessie_blake3_text"],  # "blake3:<hex>"
        v["nessie_sha256_text"],  # "sha2-256:<hex>"
        v["bare_blake3_hex"],  # bare 64-char digest hex
    ):
        with pytest.raises(ValueError):
            RawContentId.parse(dialect)
        with pytest.raises(ValueError):
            ContentId.parse(dialect)
