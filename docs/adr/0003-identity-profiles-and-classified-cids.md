# ADR 0003 — Two mintable identity profiles, one classifier, and the profile law
...
### D2 — One classifier: `ClassifiedCid { Content | Raw | Foreign(ForeignCid) }`
...
```text
   ...
   BLAKE3       BLAKE3        (sha2-256, REAPI, CIDv0, …)
      │           │                 │
  ContentId  RawContentId      ForeignCid
```
...
the wire. Algorithm agility in the classifier; no algorithm ambiguity in the
minter.

**Canonical form (revised after review).** `Foreign` wraps a validated
`ForeignCid`, not a bare `Cid`: its inner value is private and every constructor
(`new`, `TryFrom<Cid>`, `FromStr`, `from_bytes`, `Deserialize`) **rejects a
recognized profile**. With `ContentId` and `RawContentId` each admitting only
their own profile, the three are pairwise disjoint and jointly total over
well-formed CIDs — so every CID has exactly one representation, no variant can
hold a CID outside its profile, and there is no way (hand-written variant,
crafted link, human-readable string) to build a `Foreign` that aliases a
`ContentId`/`RawContentId`. Equality is therefore equality of the underlying
CID.

**Named *Classified*, not *Verified* (revised after review).** The type proves
**structural validity** (well-formed CID) and **profile classification** — it
does **not** verify content against a digest, which is `RawContentId::verify` /
`ContentAddressable::verify` and needs the bytes. "Verified" would overclaim
precisely the property a security consumer must not assume — and the word is
already taken in this crate by `VerifiedStore` (`src/store.rs`), which *does*
check content against its id on every read. Two different meanings of
"verified" in one public surface is exactly the ambiguity law 6 exists to
prevent. Renamed before the surface froze; no released name changed.
...
/ `RawContentId::from_str` / `ClassifiedCid::from_str` are never taught these
dialects. Migration conveniences must not become permanent wire protocol; the
module is expected to shrink.

The conversions are pinned as **portable conformance data**, not Rust-only unit
tests: `tests/legacy_vectors.json` (generated by `tests/gen_legacy_vectors.rs`)
carries, per input, the kyln envelope-hex, the nessie `blake3:<hex>` and
`sha2-256:<hex>` texts, the bare digest hex, and the expected `RawContentId` /
foreign CID. The Rust gate drives the adapters against it; the Python gate
checks the same identities with the API it has **and re-derives the SHA-256
digests with `hashlib`**, so the `sha2-256` rows are genuine nessie interop
values rather than shapes. The adapters stay Rust-only and default-off — giving
Python a legacy-parsing face would widen exactly the surface this decision
narrows.
...
```rust
IdentityMigration { from: ClassifiedCid, to: ClassifiedCid /* mintable */, reason: MigrationKind }
...
**Invariants are unbypassable (revised after review).** Fields are private with
accessors, and the single construction path `IdentityMigration::new` enforces
(1) `to` is mintable and (2) `from != to` (a self-migration states nothing and
would give two ids to one claim). `Deserialize` decodes a private wire shape
with `deny_unknown_fields` and then re-runs that constructor, so a decoded
record satisfies exactly what a constructed one does; with no public fields and
no setters, a valid record cannot later be mutated into an invalid one.
...
- **Additive to the `0.1.x` freeze.** `RawContentId`, `ClassifiedCid`,
  `ForeignCid`, and `ContentId::from_dag_cbor_digest` are new; nothing frozen
  changed.
...
- **Per-consumer migration checklist** (each ends with `content-addressable` as
  ...
  a `Recanonicalized` migration), nessie-store (`Digest` → `ClassifiedCid`;
...
- **`Foreign(Cid)` as a bare tuple variant.** Rejected on review (D2): a caller
  could hand-write `Foreign(some_dag_cbor_blake3_cid)`, giving one CID two
  representations and breaking the canonical form the law depends on.
  `ForeignCid` validates instead, and `match` ergonomics survive.
- **An opaque `ClassifiedCid` struct with a `kind()` accessor.** Rejected (D2):
  it would buy the same canonicality at the cost of pattern matching, which is
  the ergonomic point of classification.
